...
This page provides guidance on how to handle some common use cases with Coil. You might have to modify this code to fit your exact requir# Recipes
...
[Palette](https://developer.android.com/training/material/palette-colors?hl=en) allows you to extract prominent colors from an image. To create a `Palette`, you'll need access to an image's `Bitmap`. This can be done a number of ways:
...
imageLoader.enqueue(request)
```
ements, but it should hopefully give you a push in the right direction!
